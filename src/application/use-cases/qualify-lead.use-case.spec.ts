...
describe('QualifyLeadUseCase', () => {
  it('should qualify a lead', async () => {
    ...
    expect(qualified.email).toBe('joao@example.com');
  });
  ...
  it('should persist qualified status', async () => {
    const repository = new InMemoryLeadRepository();
    const useCase = new QualifyLeadUseCase(repository);
    ...
    await useCase.execute(lead.id);

    const savedLead = await repository.findById(lead.id);
    expect(savedLead?.status).toBe(LeadStatus.QUALIFICADO);
  });
});
