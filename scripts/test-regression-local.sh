...
echo "🧪 Executando testes de integração (Prisma repository)..."
DATABASE_URL="$DB_URL" DIRECT_URL="$DB_URL" npm test -- --runInBand src/infrastructure/database/prisma-user.repository.spec.ts

echo "🧪 Executando testes E2E completos..."
DATABASE_URL="$DB_URL" DIRECT_URL="$DB_URL" npm run test:e2e -- --runInBand

echo "✅ Regressão local concluída"